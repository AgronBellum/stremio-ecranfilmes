...
import {
  buildMediaFlowProxyExtractorRedirectUrl, guessHeightFromPlaylist,
  hasMultiEnabled, supportsMediaFlowProxy,
} from '../utils';
...
export class MegaFlix extends Extractor {
  ...
  protected async extractInternal(ctx: Context, url: URL, meta: Meta): Promise<InternalUrlResult[]> {
    ...
    // 1. Tratamento de links diretos Base64 (parâmetro link=)
    const decodedPlayer = this.tryDecodePlayerLink(targetUrl);
    if (decodedPlayer) {
      targetUrl = decodedPlayer;
    }

    // 2. Resolver wrappers específicos do MegaFlix (/cnvs/ ou /hide/)
  ...
  private async extractVidara(url: string, ctx: Context, headers: Record<string, string>): Promise<string | null> {
  ...
  private async extractViaMoonPhp(ctx: Context, packedJs: string, referer: string, headers: Record<string, string>): Promise<string | null> {
    try {
      ...
      const res = await this.fetcher.json(ctx, new URL('https://app.megafrixapi.com/moon.php'), {
        method: 'POST',
        headers: {
          ...headers,
          'Referer': referer,
...
// Helpers adicionais para String em TS
declare global {
  interface String {
